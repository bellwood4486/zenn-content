...
PostgreSQLにおける「トランザクション分離レベル」と「各分離レベルで禁止するる現象」とは何かについて簡単に整理します。
...
トランザクションがすべて直列に実行されれば、並行性の問題は回避できすます。しかしそれだと一つずつしか処理していけないためパフォーマンスを落とすことになります。 並行性のすべての問題を回避するのではなく、「一部の問題からは保護する」といったように保護の度合いを段階的に分けたものをトランザクション分離レベルと呼んでいます。
...
:::message
PostgreSQLのマニュアル(日本語版)ではトランザクション分離レベルをカタカナで表記しているため、本記事もそれに合わせています。
:::
...
「安全」と書かれている現象は発生しません。一方、「可能性あり」は発生します。  
...
また、この表に「許容されるが、PostgreSQLでは発生しない」とあります。これは「SQL標準としては発生してもよいとされているが、PostgreSQLでは実装上発生しない」というものです。  
「許容されるが、PostgreSQLでは発生しない」を「安全」と読み替えてみると、リードアンコミッティドとリードコミッティドの保護内容には差がなくなります。
...
前述した通り、PostgreSQLではこの現象は起きません。(リードアンコミッティド分離レベルでも許容しない方向倒して実装されているから)  
...
client1, client2ともに、リードコミッティド分離レベルでトランザクションを開始します。この分離レベルは前述の表で反復不可能読み取りが「可能性あり」となっているので現象を起こせるはずです。
```
...
前述の表だとこの現象はシリアライザブル分離レベル以外では「可能性あり」となっています。そのためリピータブルリード分離レベルではこの現象は起きるはずです。  
マニュアルに載っている例を使い確認していきます。まずテーブルを作成します。
```
