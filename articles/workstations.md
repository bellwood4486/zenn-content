...
[マイクロサービスのローカル開発をTiltでちょっと楽にする - HRBrain Blog](https://times.hrbrain.co.jp/entry/2021/12/01/tilt)
...
[出展] SUZUKI Yoshiharu マイクロサービスのローカル開発環境をTiltを使って2年運用して得た知見
...
- **VM Gateway経由**（HTTPSアクセス）
  ...
  🔗 [公式ドキュメント](https://cloud.google.com/workstations/docs/access-http-servers-running-on-workstations?hl=ja)
...
- **SSH Server経由**（SSHアクセス）
  ...
  🔗 [公式ドキュメント](https://cloud.google.com/workstations/docs/ssh-support?hl=ja)
...
[Google Cloud Tech Acceleration Program(TAP)参加レポート - HRBrain Blog](https://times.hrbrain.co.jp/entry/tap2023)
...
🔗 [カスタマイズの公式ドキュメント](https://cloud.google.com/workstations/docs/customize-container-images?hl=ja)

また、公式ドキュメントに加えて、以下のハンズオン資料も参考になります。こちらも分かりやすく、おすすめです。

🔗 [Google Cloud Workstations ハンズオン](https://github.com/GoogleCloudPlatform/gcp-getting-started-lab-jp/blob/master/workstations/tutorial_ws.md)
...
🔗 [Cloud Workstationsの料金ページ](https://cloud.google.com/workstations/pricing?hl=ja)
...
事業の成長に伴い、プロダクトは増え、変化していきます。同時に、開発環境もエンジニアにとって毎日使う重要なインフラの一つです。インフラも事業とともに成長しなければ、老朽化によって開発の負担が大きくなってしまいます。
